...
const CreateRoom = () => {
  ...
  const handleAddService = (id: number) => {
    const service = listService.find((item) => item._id === id)
    service!.status = !service!.status

    if (service!.status) {
      setRoom({ ...room, services: [...room.services, service!] })
    } else {
      setRoom({
        ...room,
        services: room.services.filter((item) => item._id !== id)
      })
    }
  }
  ...
  const getData = () => {
    const data = JSON.parse(getItem("rooms") || "[]")
    if (data) {
      setRooms(data)
    }
    console.log(rooms)
